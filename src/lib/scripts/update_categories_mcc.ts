...
const supabase = createClient(
  ...
  process.env.SUPABASE_SERVICE_ROLE_KEY! // needs write access under RLS
)
...
function getCategoryFromMCC (mcc?: number | null): string | null {
  if (!mcc) return null
...
async function run () {
  ...
  for (const integ of integrations ?? []) {
    ...
    const res = await fetch(
      ...
      { headers: { 'X-Token': integ.api_token } }
    )
    if (!res.ok) {
      console.warn(
        `Monobank fetch failed for wallet ${integ.wallet_id}: ${res.statusText}`
      )
      continue
    }
    const bankTxs: Array<{ id: string; mcc: number; originalMcc?: number }> =
      await res.json()
